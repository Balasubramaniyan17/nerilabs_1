...
# Secure CORS configuration: allow_credentials=False when origins='*' to prevent credential reflection
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=False,
    allow_methods=["GET", "POST", "OPTIONS"],
    allow_headers=["X-API-Key", "X-Publishable-Key", "X-Experiment-Token", "Content-Type", "Authorization"]
)
...
def startup_event():
    try:
        mab = get_or_create_mab("exp_saas_hero_conversion", "tenant_startup_01")
        analytics = mab.compute_analytics()
        if analytics.total_impressions == 0:
            simulate_traffic("exp_saas_hero_conversion", visitors=50, x_api_key="nerilabs_sk_live_9a8b7c6d5e4f3a2b1c")
    except Exception as e:
        print(f"[Startup] Default MAB init notice: {e}")
