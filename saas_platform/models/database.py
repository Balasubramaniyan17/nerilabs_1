...
class Database:
    ...
    def _seed_default_tenant(self):
        ...
        with conn:
            ...
            cur_exp = conn.execute("SELECT experiment_id FROM experiments WHERE experiment_id = 'exp_saas_hero_conversion'")
            if not cur_exp.fetchone():
                exp = Experiment(
                    experiment_id="exp_saas_hero_conversion",
                    tenant_id="tenant_startup_01",
                    title="Homepage Hero Optimization",
                ...
                try:
                    ...
                    print(f"[Database] Notice seeding initial variants: {e}")
    ...
    def list_experiments_for_tenant(self, tenant_id: str) -> List[Experiment]:
        conn = self._get_connection()
        rows = conn.execute("SELECT * FROM experiments WHERE tenant_id = ?", (tenant_id,)).fetchall()
