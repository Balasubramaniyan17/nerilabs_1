...
let CURRENT_EXP_ID = "exp_saas_hero_conversion";
...
async function fetchAnalytics() {
    try {
        ...
        // Update Dimension Performance Summary Card
        const summaryBox = document.getElementById("dimension-summary-box");
        if (currentDimensionFilter !== "ALL" && dims[currentDimensionFilter]) {
            const summary = dims[currentDimensionFilter];
            summaryBox.style.display = "flex";
            document.getElementById("dim-summary-title").textContent = `${currentDimensionFilter} Variations (${summary.total_arms} arms)`;
            document.getElementById("dim-summary-cvr").textContent = (summary.blended_cvr * 100).toFixed(2) + "%";
            document.getElementById("dim-summary-winner").textContent = summary.best_arm_name ? summary.best_arm_name : "None";
            document.getElementById("dim-summary-lift").textContent = (summary.lift_over_control_pct >= 0 ? "+" : "") + summary.lift_over_control_pct.toFixed(1) + "% vs Control";
            document.getElementById("dim-summary-lift").style.color = summary.lift_over_control_pct >= 0 ? "#10b981" : "#ef4444";
        } else {
        ...
        if (resOverall.ok) {
            const overall = await resOverall.json();
        ...
        data.filtered_arms.forEach(arm => {
            const tr = document.createElement("tr");
            const statusTag = arm.is_leading 
            ...
            tr.innerHTML = `
                ...
                <td>${arm.impressions.toLocaleString()}</td>
                <td>${arm.conversions.toLocaleString()}</td>
        ...
        console.debug("Analytics fetch error:", e);
    }
}
